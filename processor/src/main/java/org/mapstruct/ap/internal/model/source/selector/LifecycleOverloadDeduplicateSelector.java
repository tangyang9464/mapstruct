 ...
 * <p>
 * In the context of lifecycle callback method selection (such as @BeforeMapping or @AfterMapping),
 * it is possible to have multiple overloaded methods whose parameter lists are structurally identical except for the specific types,
 * where those types are related by inheritance (e.g., one parameter is a superclass or subclass of another).
 * <p>
 * This selector groups such methods by their effective parameter signature (ignoring differences only in type hierarchy),
 * and, within each group, retains only the method whose parameter types have the closest inheritance distance to the actual invocation types.
 * This ensures that, for each group of nearly identical overloads, only the most specific and appropriate method is selected.
 * <p>
...
public class LifecycleOverloadDeduplicateSelector implements MethodSelector {
    ...
    /**
     * Builds a grouping key for a method based on its defining type, method name, and a detailed breakdown of each parameter binding.
     * The key consists of:
     * <ul>
     ...
     * </ul>
     * This ensures that methods are grouped together only if all these aspects match, except for differences in type hierarchy, which are handled separately.
     */
